...
#[command(
    ...
    about = "A laptop-oriented scheduler focused on efficiency and interactivity.",
    long_about = r#"
scx_lnl is a laptop-oriented scheduler (Lunar Lake-focused) that focuses on efficiency and everyday responsiveness.
...
struct Opts {
    ...
    /// Disable energy-model-based CPU selection.
    ///
    /// Energy-aware CPU selection is enabled by default and uses userspace-provided energy model
    /// inputs (`cpu_capacity` / `cpu_energy_cost`) to rank idle CPUs.
    #[clap(long, action = clap::ArgAction::SetTrue)]
    no_energy_aware: bool,
    ...
    /// Specifies the preferred set of CPUs for interactive bursts.
    ///
    /// This is typically set to "performance" (P-cores) on hybrid systems.
    ///
    /// Special values:
    ...
    /// Voluntary context switch rate threshold used as a heuristic to classify tasks as interactive.
    #[clap(long, default_value = "4")]
    interactive_nvcsw_thresh: u64,

    /// Boost window after wakeup for interactive tasks (milliseconds).
    ...
    /// Energy model scaling factor (percent) applied to little-core energy costs.
    #[clap(long, default_value = "100")]
    em_little_cost_pct: u32,
...
impl<'a> Scheduler<'a> {
    ...
    fn init(opts: &'a Opts, open_object: &'a mut MaybeUninit<OpenObject>) -> Result<Self> {
        ...
        let perf_domain = Self::resolve_perf_domain(&opts.perf_domain).map_err(|err| {
            anyhow!(
                "failed to resolve perf domain '{}': {}",
                &opts.perf_domain,
                err
            )
        })?;
        ...
        // Set scheduler flags.
        skel.struct_ops.ext_ops_mut().flags = *compat::SCX_OPS_ENQ_EXITING
        ...
        info!(
            "scheduler flags: {:#x}",
            skel.struct_ops.ext_ops_mut().flags
        );

        // Load the BPF program for validation.
        let mut skel = scx_ops_load!(skel, ext_ops, uei)?;

        // Initialize the primary and performance CPU domains.
        ...
        Self::init_perf_domain(&mut skel, &perf_domain).map_err(|err| {
            anyhow!(
                "failed to initialize perf domain 0x{:x}: {}",
                perf_domain,
                err
            )
        })?;

        Self::init_energy_model_data(
            &mut skel,
            &topo,
            opts.em_big_cost_pct,
        ...
        let struct_ops = Some(scx_ops_attach!(skel, ext_ops)?);
    ...
    fn enable_perf_cpu(skel: &mut BpfSkel<'_>, cpu: i32) -> Result<(), u32> {
        let prog = &mut skel.progs.enable_perf_cpu;
        let mut args = cpu_arg {
            cpu_id: cpu as c_int,
        };
        let input = ProgramInput {
    ...
    fn resolve_perf_domain(perf_domain: &str) -> Result<Cpumask> {
        let domain = match perf_domain {
            "turbo" => Self::epp_to_cpumask(Powermode::Turbo)?,
            "performance" => Self::epp_to_cpumask(Powermode::Performance)?,
            "all" => Self::epp_to_cpumask(Powermode::Any)?,
            &_ => Cpumask::from_str(perf_domain)?,
        };
    ...
    fn init_perf_domain(skel: &mut BpfSkel<'_>, domain: &Cpumask) -> Result<()> {
        info!("perf CPU domain = 0x{:x}", domain);

        // Clear the perf domain by passing a negative CPU id.
        if let Err(err) = Self::enable_perf_cpu(skel, -1) {
            bail!("failed to reset perf domain: error {}", err);
        }

        for cpu in 0..*NR_CPU_IDS {
            if domain.test_cpu(cpu) {
                if let Err(err) = Self::enable_perf_cpu(skel, cpu as i32) {
                    bail!("failed to add CPU {} to perf domain: error {}", cpu, err);
                }
            }
        }
    ...
    fn init_energy_model_data(
        ...
        let em = match EnergyModel::new() {
            Ok(em) => em,
            Err(err) => {
                warn!("energy model not available: {}", err);
                return Ok(());
            }
        };
    ...
    fn init_cpufreq_perf(skel: &mut BpfSkel<'_>, enabled: bool) -> Result<()> {
        // cpufreq_perf_lvl semantics:
        //   -2: disable cpuperf control entirely
        //   -1: dynamic cpuperf control (intended for schedutil)
        //  >=0: fixed cpuperf level
        let perf_lvl: i64 = if enabled { -1 } else { -2 };
        info!(
