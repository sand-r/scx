 ...
 *
 * @spill_thresh: depth the node's shared DSQ must reach before an idle CPU
 * outside the primary domain is woken up (0 = spill immediately). Overflowing
 * as soon as the primary domain is busy wakes a core in the other cluster even
 * for a task that only runs briefly, and on a hybrid laptop that means powering
 * up the cluster and its cache for very little work. Requiring a backlog first
 * keeps transient bursts on the primary domain, at the cost of waiting up to a
 * slice for a CPU there. Note the DSQ is shared by every CPU on the node, so
 * this is a measure of how backed up the node is rather than a count of tasks
 * specifically waiting for the primary domain. Only honored when NUMA is
 * disabled; see pick_idle_cpu().
 *
 ...
 */
private(LNL) struct bpf_cpumask __kptr *primary_cpumask;
...
static s32 pick_idle_cpu(struct task_struct *p, s32 prev_cpu, u64 wake_flags, bool *is_idle)
{
	...
	if (cpu < 0) {
		/*
		 * The primary domain is busy. Unless the node is already
		 * backed up, leave @p queued rather than waking a CPU outside
		 * the domain: the primary CPUs are running and will pick it up
		 * as they come out of their slices, whereas overflowing here
		 * powers up an idle CPU in the other cluster.
		 *
		 * Restricted to the single-node case. Returning without an
		 * idle CPU sends @p to the DSQ of prev_cpu's node and skips
		 * the kick, since a CPU was nominally selected; with several
		 * nodes the primary CPUs may consume a different DSQ, leaving
		 * @p queued until something else wakes a CPU on this one.
		 *
		 * Tasks that cannot use the primary domain at all are not
		 * subject to this and overflow immediately.
		 */
		if (spill_thresh && numa_disabled && !primary_all && primary &&
 ...
 */
s32 BPF_STRUCT_OPS(lnl_select_cpu, struct task_struct *p,
 ...
 * select_cpu().
 */
void BPF_STRUCT_OPS(lnl_enqueue, struct task_struct *p, u64 enq_flags)
{
...
void BPF_STRUCT_OPS(lnl_dispatch, s32 cpu, struct task_struct *prev)
{
...
void BPF_STRUCT_OPS(lnl_running, struct task_struct *p)
{
	struct task_ctx *tctx;
...
void BPF_STRUCT_OPS(lnl_enable, struct task_struct *p)
{
	if (rr_sched)
		return;
...
void BPF_STRUCT_OPS(lnl_cgroup_set_weight, struct cgroup *cgrp, u32 weight)
{
	struct cgrp_ctx *cgc;
...
void BPF_STRUCT_OPS(lnl_cgroup_move, struct task_struct *p,
...
s32 BPF_STRUCT_OPS(lnl_init_task, struct task_struct *p,
...
/*
 * When the CPU is taken by a higher-priority scheduler class, re-enqueue
 * the local DSQ so the preempted tasks get a chance to run elsewhere.
 */
void BPF_STRUCT_OPS(lnl_cpu_release, s32 cpu, struct scx_cpu_release_args *args)
{
...
/*
 * Pick a CPU to kick for keeping the sched_ext watchdog alive.
 * Distributes kicks across online CPUs to avoid pinning any one core.
 */
static s32 watchdog_kick_cpu(void)
{
	...
	if (!online)
		return 0;
	cpu = bpf_cpumask_any_distribute(online);
	scx_bpf_put_cpumask(online);

	return cpu < nr_cpu_ids ? cpu : 0;
}

/*
 * Watchdog timer: periodically kicks an idle CPU so the sched_ext watchdog
 * doesn't misinterpret sustained idle as a runnable stall.
 */
...
s32 BPF_STRUCT_OPS_SLEEPABLE(lnl_init)
{
	struct bpf_timer *timer;
	int err, node;
	u32 key = 0;
...
void BPF_STRUCT_OPS(lnl_exit, struct scx_exit_info *ei)
{
	UEI_RECORD(uei, ei);
}

SCX_OPS_DEFINE(lnl_ops,
	       .select_cpu		= (void *)lnl_select_cpu,
	       .enqueue			= (void *)lnl_enqueue,
	       .dispatch		= (void *)lnl_dispatch,
	       .running			= (void *)lnl_running,
	       .stopping		= (void *)lnl_stopping,
	       .runnable		= (void *)lnl_runnable,
	       .cpu_release		= (void *)lnl_cpu_release,
	       .enable			= (void *)lnl_enable,
	       .cgroup_init		= (void *)lnl_cgroup_init,
	       .cgroup_set_weight	= (void *)lnl_cgroup_set_weight,
	       .cgroup_move		= (void *)lnl_cgroup_move,
	       .init_task		= (void *)lnl_init_task,
	       .init			= (void *)lnl_init,
	       .exit			= (void *)lnl_exit,
	       .timeout_ms		= 5000,
	       .name			= "scx_lnl");
