...
/*
 * Voluntary context switch rate threshold to treat a task as interactive.
 *
 * Higher values make the scheduler less eager to treat bursty tasks as
 * interactive.
 */
const volatile u64 interactive_nvcsw_thresh = 4ULL;

/*
 * Boost window after wakeup for interactive tasks.
 */
 ...
 */
private(EXT) struct bpf_cpumask __kptr *primary_cpumask;

/*
 * Preferred CPUs for interactive bursts (typically P-cores).
 */
 ...
 */
static s32 pick_idle_cpu_builtin(struct task_struct *p, const struct task_ctx *tctx,
			 ...
			 s32 prev_cpu, u64 wake_flags, bool *is_idle)
{
	 ...
	 */
	cpu = pick_idle_cpu_builtin(p, tctx, prev_cpu, wake_flags, is_idle);
	if (cpu >= 0)
		return cpu;
 ...
 */
s32 BPF_STRUCT_OPS(ext_select_cpu, struct task_struct *p,
...
static bool keep_running(const struct task_struct *p, s32 cpu)
{
	...
	if (primary && bpf_cpumask_intersects(primary, p->cpus_ptr) &&
		...
		tctx = try_lookup_task_ctx(p);
		if (!perf || !tctx || !is_interactive(p, tctx))
			return false;
...
void BPF_STRUCT_OPS(ext_running, struct task_struct *p)
{
	u64 now = scx_bpf_now();
	struct task_ctx *tctx;
	...
	/*
	 * Boost quickly on wakeups for interactive tasks.
	 */
	if (cpufreq_perf_lvl == -1 &&
 ...
 */
void BPF_STRUCT_OPS(ext_stopping, struct task_struct *p, bool runnable)
{
...
void BPF_STRUCT_OPS(ext_runnable, struct task_struct *p, u64 enq_flags)
{
	const struct task_struct *current = (void *)bpf_get_current_task_btf();
	u64 now = scx_bpf_now();
	struct task_ctx *tctx;
...
void BPF_STRUCT_OPS(ext_quiescent, struct task_struct *p, u64 deq_flags)
{
	u64 now = scx_bpf_now();
	s64 delta_t;
	struct task_ctx *tctx;
...
void BPF_STRUCT_OPS(ext_cpu_release, s32 cpu, struct scx_cpu_release_args *args)
{
	/*
...
void BPF_STRUCT_OPS(ext_set_cpumask, struct task_struct *p,
...
void BPF_STRUCT_OPS(ext_enable, struct task_struct *p)
{
	/*
...
s32 BPF_STRUCT_OPS(ext_init_task, struct task_struct *p,
...
SEC("syscall")
int enable_perf_cpu(struct cpu_arg *input)
{
	struct bpf_cpumask *mask;
	int err = 0;

	/* Make sure the perf CPU mask is initialized */
	err = init_cpumask(&perf_cpumask);
	if (err)
		return err;

	/*
	 * Enable the target CPU in the perf domain. If the target CPU is a
	 * negative value, clear the whole mask (this can be used to reset the
	 * domain).
	 */
	bpf_rcu_read_lock();
	mask = perf_cpumask;
...
static void init_cpuperf_target(void)
{
	...
	bpf_for (cpu, 0, nr_cpu_ids) {
		 ...
		 *
		 * For dynamic control, start from the floor (avoid a high idle
...
s32 BPF_STRUCT_OPS_SLEEPABLE(ext_init)
{
	struct bpf_timer *timer;
	int err, node;
	s32 cpu;
	...
	/* Initialize the perf scheduling domain */
	err = init_cpumask(&perf_cpumask);
	if (err)
		return err;
...
void BPF_STRUCT_OPS(ext_exit, struct scx_exit_info *ei)
{
	UEI_RECORD(uei, ei);
}

SCX_OPS_DEFINE(ext_ops,
	       .select_cpu		= (void *)ext_select_cpu,
	       .enqueue			= (void *)ext_enqueue,
	       .dispatch		= (void *)ext_dispatch,
	       .running			= (void *)ext_running,
	       .stopping		= (void *)ext_stopping,
	       .runnable		= (void *)ext_runnable,
	       .quiescent		= (void *)ext_quiescent,
	       .cpu_release		= (void *)ext_cpu_release,
	       .set_cpumask		= (void *)ext_set_cpumask,
	       .enable			= (void *)ext_enable,
	       .init_task		= (void *)ext_init_task,
	       .init			= (void *)ext_init,
	       .exit			= (void *)ext_exit,
	       .timeout_ms		= 5000,
	       .name			= "scx_lnl");
